...
SCHEMA = {
    "invoice_number": ["invoice number", "inv no", "invoice #", "bill number", "invoice"],
    "invoice_date": ["invoice date", "date of issue", "billing date", "date"],
    "total_amount": ["total amount", "amount due", "total due", "balance due", "total"],
}
...
def extract_near_keyword(lines: Dict[int, List[Token]], keywords: List[str]) -> str:
    ...
    for lid, toks in lines.items(): 
        ...
        start_i = idx[2]
        value = re.sub(r'^[A-Za-z\s]+:\s*', '', " ".join(t.text for t in toks[start_i+1:])).strip(": ").strip()
        ...
        for i, (lid, toks) in enumerate(sorted_lines[:-1]): 
            ...
            value = re.sub(r'^[A-Za-z\s]+:\s*', '', " ".join(t.text for t in next_toks)).strip(": ").strip()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python doc-intelligence-2-fix.py <path-to-image-or-pdf>")
        sys.exit(1)
